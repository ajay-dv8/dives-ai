...
// get the current chatbot configuration based on domain ID
...
// store customer's email if extracted
...
export const onAiChatBotAssistant = async (
  ...
  try {
    // Fetch chatbot domain details based on the provided ID
    ...
    if (chatBotDomain) {
      ...
      // if der is an extracted email (email from customer)
      if (extractedEmail) {
        customerEmail = extractedEmail[0]
      }

      if (customerEmail) {
        // if customer email is found, then we need to get the chatbot domain
        ...
        if (checkCustomer && !checkCustomer.customer.length) {
          ...
          // if its a new customer send yo sup and user email message
          // TODO: replace yo sup wit a better welcome message
          if (newCustomer) {
            console.log('got a new customer')
            console.log('test see', checkCustomer?.customer[0].id);
            const response = {
              role: 'assistant',
              content: `hello ${
      ...
      // for anonymous customers or new customers 
      const chatCompletion = await openai.chat.completions.create({
        messages: [
          {
            role: 'assistant',
